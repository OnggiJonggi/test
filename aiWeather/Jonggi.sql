...
CREATE TABLE VC_MEMBER (
    ...
    CONSTRAINT FK_VC_MEMBER_USER
        FOREIGN KEY (USER_NO) REFERENCES MEMBER(USER_NO),
    CONSTRAINT CK_STATUS     CHECK (STATUS IN ('Y', 'N', 'D')),
    CONSTRAINT CK_ROLE_TYPE  CHECK (ROLE_TYPE IN ('S', 'A', 'N')),
);
...
COMMENT ON COLUMN VC_MEMBER.STATUS IS '상태(Y:정상, N:비활성화, D:삭제됨)';
COMMENT ON COLUMN VC_MEMBER.ROLE_TYPE IS '관리 권한(S:방장, A:관리자, N:일반)';